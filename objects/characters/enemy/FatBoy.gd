...
func _physics_process(delta):
	if Global.player.get_global_position().distance_to(get_global_position()) <= 3000:
		...
		$Enemy2.look_at(Global.player.get_global_position())
		if not immobile:
...
func _on_Timer_timeout():
	if Global.player.get_global_position().distance_to(get_global_position()) <= 5000:
		...
		b.set_global_position($Enemy2/Position2D.get_global_position())
		b.set_global_rotation($Enemy2/Position2D.get_global_rotation())
